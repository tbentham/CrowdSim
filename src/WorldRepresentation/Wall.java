...
public class Wall implements BuildingObject {
    ...
    public boolean intersects(Point2d p1, Point2d p2, double addedLength) {
    	...
    	if (divisor == 0)
    		return false;	// 0 => parallel lines, most likely no intersect
    ...
    public boolean intersects(Wall w, double addedLength) {
        return intersects(new Point2d(w.getStartVector().x, w.getStartVector().y),
                new Point2d(w.getEndVector().x, w.getEndVector().y), addedLength);
    }

    public boolean intersects(Wall w) {
        return intersects(new Point2d(w.getStartVector().x, w.getStartVector().y),
                new Point2d(w.getEndVector().x, w.getEndVector().y), 0.0);
    }
